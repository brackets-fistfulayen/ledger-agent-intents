...
async function handleSend(args) {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log(`   Chain:     ${chainId === 1 ? 'Ethereum' : chainId === 137 ? 'Polygon' : chainId === 8453 ? 'Base' : chainId}`);
  ...
  try {
    ...
    if (data.success) {
      ...
      for (const i of data.intents) {
        ...
        console.log(`   ${i.details.amount} ${i.details.token} → ${i.details.recipient.slice(0, 10)}...`);
